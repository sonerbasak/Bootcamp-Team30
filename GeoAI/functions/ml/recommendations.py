# GeoAI/functions/ml/recommendations.py
import numpy as np
from sklearn.cluster import KMeans
from sklearn.preprocessing import StandardScaler
import math
import random
import logging # Logging için import edildi
import json # Eğer JSON operasyonları yapılacaksa

# Gerekli database fonksiyonlarını import ediyoruz
...
# Logging yapılandırması (eğer main.py'de global olarak yapılmadıysa burada da olabilir)
...
def get_recommended_friends_kmeans(current_user_id: int, num_recommendations: int = 5, num_clusters: int = 5):
    ...
    # Mevcut kullanıcının verisi yoksa veya hiç başka kullanıcı yoksa
    ...
    # Kullanıcı ID'lerini ve vektörlerini NumPy dizisine dönüştür
    user_ids = list(all_users_category_vectors.keys())
    user_vectors = np.array(list(all_users_category_vectors.values()))

    # K-Means Kümeleme için yeterli kullanıcı var mı?
    # K-Means'in çalışabilmesi için num_clusters'tan fazla kullanıcı olmalı ve en az 2 kullanıcı olmalı.
    if len(all_users_category_vectors) >= num_clusters and len(all_users_category_vectors) > 1:
        try:
            # Veriyi ölçekle (K-Means mesafeye duyarlı olduğu için önemlidir)
            scaler = StandardScaler()
            scaled_user_vectors = scaler.fit_transform(user_vectors)

            # K-Means modelini oluştur ve eğit
            # n_init='auto' kullanmak, scikit-learn'ün varsayılan değerini seçmesini sağlar (şu an 10)
            kmeans = KMeans(n_clusters=num_clusters, random_state=42, n_init='auto')
            kmeans.fit(scaled_user_vectors)

            # Her kullanıcı için küme atamasını al
            cluster_labels = kmeans.labels_
            user_cluster_map = {user_ids[i]: cluster_labels[i] for i in range(len(user_ids))}

            # Mevcut kullanıcının kümesini bul
            current_user_cluster = user_cluster_map[current_user_id]
            
            # Aynı kümedeki diğer kullanıcıları topla
            ...
            # Eğer aynı kümede öneri varsa, bunları öncelikli olarak döndür
            if friends_in_same_cluster:
                # Kullanıcı adlarını almak için users.db'ye bağlan
                # BURADA get_db_connection'ı WITH İFADESİYLE KULLANIYORUZ
                with get_db_connection(settings.USERS_DATABASE_FILE) as user_conn:
                    user_cursor = user_conn.cursor()
                    
                    recommended_users_info = []
                    # Sadece mevcut kullanıcının henüz takip etmediği kişileri öner
                    # Bu kontrolü burada yapmak, daha doğru öneriler sunar
                    for uid in friends_in_same_cluster:
                        # is_following fonksiyonu ile takip edip etmediğimizi kontrol et
                        # get_user_by_id ile kullanıcı bilgilerini çek
                        user_info = get_user_by_id(uid)
                        if user_info:
                            # Takip kontrolü için user_info["id"] kullanıldı
                ...
                # Yeterli öneri varsa K-Means sonuçlarını döndür
                if len(recommended_users_info) >= num_recommendations:
                    # Rastgele seçim yap, çünkü aynı kümede çok fazla kişi olabilir
            ...
            # K-Means başarısız olursa Kosinüs Benzerliğine düşer

    # K-Means çalışmadıysa veya yeterli öneri bulamadıysa:
    # Tüm diğer kullanıcılar arasında Kosinüs Benzerliği (eşiksiz) kullanarak öneri yap.
    ...
    with get_db_connection(settings.USERS_DATABASE_FILE) as user_conn:
        ...
        for user_id in other_user_ids:
            ...
            if user_vector:
                ...
                if not already_following: # Sadece takip edilmeyenleri değerlendir
    ...
    # num_recommendations kadar kullanıcı al, benzerlik ne olursa olsun
    top_recommendations_ids = similarities[:num_recommendations]

    recommended_users_info = []
    # BURADA get_db_connection'ı WITH İFADESİYLE KULLANIYORUZ (iç içe olmaması daha iyi)
    with get_db_connection(settings.USERS_DATABASE_FILE) as user_conn:
        user_cursor = user_conn.cursor()
